...
ip_address = "169.254.157.47"      #IDC
#ip_address = "169.254.1.99"         #IDCevo
screen = True
amount = 1000000000
csv = ''
base_filename = r'E:\DLT_Traces\analysisTraces\test'
excel_filename = r'E:\DLT_Traces\analysisTraces\error_counts.xlsx'
# Start logging dlt with variables provided
def start_logging(filename):
    exportOptions = DltExport(None, screen, csv, filename, amount)
    c = connector.DltConnectorThread(ip_address, exp=exportOptions, buffering=False)
    c.start_reading()
    return c
...
while True:
    ...
    # The time for the logging process 
    time.sleep(120)
    stop_logging(c)
