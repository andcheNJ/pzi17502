# -*- coding: utf-8 -*-
"""
Created on Fri Feb 23 14:17:53 2024
...
#
# @file
# @copyright Copyright (C) 2018-2022 BMW Car IT GmbH
#
...
def main():
    ...
    parser.add_argument(
        'UDS',
        help="UDS data, as a list of bytes in hex (e.g., 01 25 FF ...)",
        nargs="+",
    ...
    if args.ip_addr:
        target = args.ip_addr.strip().rsplit(":", 1)
    ...
    server.send(
        ...
        + bytes([TESTER_TOOL_ADDRESS, args.diag_addr])
        + bytes(args.UDS)
    )
        ...
        while len(data_buffer) >= HSFZ_HEADER_LEN:
            ...
            if control == CONTROL_DIAGNOSIS:
                ...
                if len(payload) != (DIAGNOSIS_HEADER_LEN + NRC_MSG_LEN) \
                    ...
                    print(" ".join("%02X" % x for x in payload[DIAGNOSIS_HEADER_LEN:]))
                    server.close()
                    server = None
                    response_found = True
